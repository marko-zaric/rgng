...
# Da = scipy.io.loadmat('D1data.mat')
# INPUT = Da["X"]

# files = os.listdir("/home/marko/Desktop/index_lists/")
# rand_states = []
# for i in range(1, 13):
#     rand_states.append(np.loadtxt("/home/marko/Desktop/index_lists/"+ str(i) + ".mat"))
...
class RobustGrowingNeuralGas:
    ...
    def fit_network(self, a_max, passes=10):
        ...
        while len(self.network.nodes()) <= self.prenumnode and stopflag == 1:
            print("Training when the number of the nodes in RGNG is: ", len(self.network.nodes()), ' ...')
            flag = 1
            self.old_prototypes = []
            harmdist=[]
