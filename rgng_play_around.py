...
class RobustGrowingNeuralGas:
    ...
    def fit_network(self, e_b, e_n, a_max, l, a, d, passes=1, plot_evolution=False):
        ...
        print(len(np.minimum(self.data, self.data))) # What's happening here?
        ...
        for p in range(passes):
            ...
            # (b)1) Compute initial restricting distance for each prototype (as harmonic mean of error)
            ...
            for observation in self.data:
                ...
                # 2./(c2) find the nearest unit s_1 and the second nearest unit s_2
                ...
                # print("s_1",s_1)
                # print(self.network.nodes(data=True))
                #e_b = self.e_bi * pow((self.e_bf / self.e_bi ), self.prenode_rank[s_1] / self.max_nodes) 007
                #e_b = self.e_bi * pow((self.e_bf / self.e_bi ), self.network.nodes[s_1]["prenode_ranking"] / self.max_nodes)
                #print("s1, eb: ",s_1,e_b)
                # prototype update
                ...
                for neighbor in self.network.neighbors(s_1):
                    ...
                    # ------
                    # adaptive rate
                    # e_n = self.e_ni * pow((self.e_nf / self.e_ni ), self.prenode_rank[neighbor] / self.max_nodes) 007
                    #e_n = self.e_ni * pow((self.e_nf / self.e_ni ), self.network.nodes[neighbor]["prenode_ranking"] / self.max_nodes)
                ...
                if steps % l == 0:
                    ...
                    for u in self.network.nodes():
                        if u in self.receptive_field:
                            harmdist_i = self.h_mean([np.linalg.norm(x - self.network.nodes[u]['vector']) for x in self.receptive_field[u]['input']])
                            self.network.nodes[u]['error'] = np.sum([ np.exp(-np.linalg.norm(x - self.network.nodes[u]['vector'])/harmdist_i)*np.linalg.norm(x - self.network.nodes[u]['vector']) for x in self.receptive_field[u]['input']])
                    ...
                    # initialize the error variable of r with the new value of te_he error variable of q
            ...
            # pp.pprint(self.receptive_field)
            # 10. Update MDL
